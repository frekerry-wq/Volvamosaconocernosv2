...
function renderCounter(){
  ...
  unitsBox.innerHTML = units.map(([n,l])=>
...
function renderQuestion(){
  ...
  curPicked = null;
  document.getElementById('qLevel').textContent = LEVEL_NAMES[curLevel];
  document.getElementById('qText').textContent = item.q;
  document.getElementById('qProgress').style.width = ((curIndex)/set.length*100)+'%';
  ...
  item.o.forEach(text=>{
    ...
    b.addEventListener('click',()=>{
      ...
      curPicked = text;
      document.getElementById('qOpen').classList.remove('hidden');
      document.getElementById('nextQ').classList.remove('hidden');
    });
    optsBox.appendChild(b);
  });
  document.getElementById('qOpen').value = '';
  document.getElementById('qOpen').classList.add('hidden');
...
document.getElementById('nextQ').addEventListener('click',()=>{
  ...
  if(curPicked || written){
    ...
    document.getElementById('quizCard').classList.add('saved-flash');
    setTimeout(()=>document.getElementById('quizCard').classList.remove('saved-flash'),800);
  }
  curIndex++;
...
const WHEEL_COLORS = ['rgba(143,180,255,.35)','rgba(255,158,196,.35)','rgba(199,156,255,.35)','rgba(143,230,180,.35)'];
...
const TIMELINE = [
  ...
  {title:"Nuestra primera cita", text:"(Escribe aqui como fue, a donde fueron, que sentiste.)"},
  {title:"El primer 'te amo'", text:"(Cuenta como paso, quien lo dijo primero.)"},
  {title:"Un viaje o momento especial", text:"(Ese recuerdo que los dos atesoran.)"},
  {title:"Hoy", text:"Mas de dos anos despues, aqui seguimos. Y lo que falta."},
];
